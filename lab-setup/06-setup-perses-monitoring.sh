#!/bin/bash
#
# RHACS monitoring: COO stack, Perses, Prometheus scrape, RHACS UserPKI auth + verification
# Merged from rhacs-demo/monitoring-setup (install.sh, 01-setup-certificates.sh,
# 02-install-monitoring.sh, 03-configure-rhacs-auth.sh). YAMLs: <repo>/monitoring-setup/
#
# Requires: oc, openssl, envsubst; ROX_CENTRAL_ADDRESS + ROX_API_TOKEN (e.g. from ~/.bashrc)
#
...
MONITORING_DIR="$PROJECT_ROOT/monitoring-setup"
if [[ ! -d "$MONITORING_DIR" ]]; then
  echo "[MONITORING] ERROR: monitoring-setup not found: $MONITORING_DIR" >&2
  exit 1
fi
cd "$MONITORING_DIR"
...
BLUE='\033[0;34m'
NC='\033[0m'
log() { echo -e "${GREEN}[MONITORING]${NC} $1"; }
warn() { echo -e "${YELLOW}[MONITORING]${NC} $1"; }
error() { echo -e "${RED}[MONITORING]${NC} $1" >&2; }
step() { echo -e "${BLUE}[MONITORING]${NC} $1"; }

get_rox_endpoint() {
  local url="${ROX_CENTRAL_ADDRESS:-}"
  echo "${url#https://}"
}
print_rerun() { log "To rerun this step: bash \"$1\""; }
...
echo "=============================================="
echo ""
log "Starting installation (work dir: $MONITORING_DIR)..."
echo ""
load_rox_from_bashrc
MISSING_VARS=0
...
MONITORING_STACK_YAML="cluster-observability-operator/monitoring-stack.yaml"
SCRAPE_CONFIG_YAML="cluster-observability-operator/scrape-config.yaml"
...
if oc get crd prometheuses.monitoring.coreos.com &>/dev/null; then
  PROM_OP_PROM_YAML="prometheus-operator/prometheus.yaml"
  oc apply -f prometheus-operator/
...
UI_PLUGIN_YAML="perses/ui-plugin.yaml"
DATASOURCE_YAML="perses/datasource.yaml"
DASHBOARD_YAML="perses/dashboard.yaml"
...
if [ -n "$AUTH_PROVIDER_ID" ]; then
  ...
  GROUP_PAYLOAD=$(envsubst < rhacs/admin-group.json.tpl)
...
echo "============================================"
echo ""
log "Loading ROX_* from ~/.bashrc before verification..."
load_rox_from_bashrc
...
if [ -n "$GROUPS_LIST" ]; then
  ...
  if echo "$AUTH_TEST" | grep -q '"userId"'; then
    ...
    if echo "$METRICS_TEST" | grep -q "access for this user is not authorized"; then
      ...
      echo "  optional: see rhacs-demo/monitoring-setup/troubleshoot-auth.sh if copied into this repo"
