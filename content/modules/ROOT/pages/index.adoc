...
The setup script configures required bastion environment variables (including RHACS client settings) before running the rest of the install.

*Procedure*

[start=1]
. Clone the repository and run the parallel setup driver to configure your environment (lab automation, Trusted Artifact Signer, and OpenShift AI checks run at the same time with a progress bar).
+
[IMPORTANT]
====
